...
// ── CRASH SAFETY NETS ────────────────────
// Never let a single bad promise or unexpected error take down the whole server.
// Log it and keep serving other requests instead of crashing the process.
...
// Build a MongoDB filter object from query-string filter params.
// Shared by Dashboard, Reports, and Document Register so filtering behaves
// identically everywhere. `extra` can add page-specific constraints (e.g. contractor lock).
function buildDocFilter(req, extra) {
  const filter = Object.assign({}, extra || {});
  if (req.query.type)       filter.type       = req.query.type;
  if (req.query.discipline) filter.discipline = req.query.discipline;
  if (req.query.status)     filter.status     = req.query.status;
  if (req.query.contractor && !filter.contractor) filter.contractor = req.query.contractor;
...
app.get('/api/stats', requireAuth, async (req, res) => {
  try {
    ...
    const [total, transmittals, overdue, submittedToday, submittedWeek, submittedMonth,
      ...
      Document.countDocuments(df),
      Transmittal.countDocuments(),
      Document.countDocuments(Object.assign({},df,{ due_date: { $lt: today }, status: { $nin: ['Approved','Closed','Cancelled'] } })),
      Document.countDocuments(Object.assign({},df,{ created_at: { $gte: startOfDay } })),
      Document.countDocuments(Object.assign({},df,{ created_at: { $gte: startOfWeek } })),
      Document.countDocuments(Object.assign({},df,{ created_at: { $gte: startOfMonth } })),
      Document.countDocuments(Object.assign({},df,{ status: 'Approved' })),
      Document.countDocuments(Object.assign({},df,{ status: 'Rejected' })),
      Document.countDocuments(Object.assign({},df,{ status: { $in: ['Pending','Open','Under Review'] } })),
      Document.countDocuments(Object.assign({},df,{ status: { $in: ['Closed','Cancelled'] } })),
      Document.countDocuments(Object.assign({},df,{ status: { $nin: ['Closed','Cancelled','Approved'] } })),
      Document.aggregate([{ $match: df }, { $group: { _id: '$status', count: { $sum: 1 } } }]),
      Document.aggregate([{ $match: df }, { $group: { _id: '$type',   count: { $sum: 1 } } }]),
      Document.aggregate([{ $match: df }, { $group: { _id: '$discipline', count: { $sum: 1 } } }]),
      Document.aggregate([{ $match: df }, { $group: { _id: { type: '$type', discipline: '$discipline', status: '$status' }, count: { $sum: 1 } } }])
    ]);
    ...
    // Monthly trend (last 12 months) — respects filter too
    const monthlyAgg = await Document.aggregate([
      { $match: Object.assign({},df,{ created_at: { $gte: moment().subtract(12,'months').toDate() } }) },
...
async function appHandler(req, res) {
  try {
    ...
    if (page === 'dashboard') {
      const today = new Date();
      const df = buildDocFilter(req);
      const hasFilter = Object.keys(df).length > 0;
      ...
      [data.total_documents,data.total_transmittals,data.total_users,data.overdue_count,
       ...
       data.submitted_today,data.submitted_week,data.submitted_month,data.active_count] = await Promise.all([
        Document.countDocuments(df),Transmittal.countDocuments(),User.countDocuments(),
        Document.countDocuments(Object.assign({},df,{due_date:{$lt:today},status:{$nin:['Approved','Closed','Cancelled']}})),
        Document.countDocuments(Object.assign({},df,{status:'Approved'})),Document.countDocuments(Object.assign({},df,{status:'Rejected'})),
        Document.countDocuments(Object.assign({},df,{status:'Under Review'})),Document.countDocuments(Object.assign({},df,{status:'Revise & Resubmit'})),
        Document.countDocuments(Object.assign({},df,{created_at:{$gte:moment().startOf('day').toDate()}})),
        Document.countDocuments(Object.assign({},df,{created_at:{$gte:moment().startOf('week').toDate()}})),
        Document.countDocuments(Object.assign({},df,{created_at:{$gte:moment().startOf('month').toDate()}})),
        Document.countDocuments(Object.assign({},df,{status:{$nin:['Closed','Cancelled','Approved']}}))
      ]);
      data.approval_rate=data.total_documents?Math.round(data.approved_count/data.total_documents*100):0;
      data.recent_docs=await Document.find(df).sort({updated_at:-1}).limit(10);
      ...
      const lockContractor = (page==='my_submissions'&&user.role==='contractor') ? { contractor: user.company } : null;
      const filter = buildDocFilter(req, lockContractor);
      data.has_filter = Object.keys(filter).length > (lockContractor?1:0);
      ...
      const today=new Date();
      const df = buildDocFilter(req);
      data.has_filter = Object.keys(df).length > 0;
      ...
      [data.total_documents,data.total_transmittals,data.overdue_count,
       data.approved_count,data.rejected_count,data.review_count,data.rr_count] = await Promise.all([
        Document.countDocuments(df),Transmittal.countDocuments(),
        Document.countDocuments(Object.assign({},df,{due_date:{$lt:today},status:{$nin:['Approved','Closed','Cancelled']}})),
        Document.countDocuments(Object.assign({},df,{status:'Approved'})),Document.countDocuments(Object.assign({},df,{status:'Rejected'})),
        Document.countDocuments(Object.assign({},df,{status:'Under Review'})),Document.countDocuments(Object.assign({},df,{status:'Revise & Resubmit'}))
      ]);
      data.approval_rate=data.total_documents?Math.round(data.approved_count/data.total_documents*100):0;
      data.overdue_docs=await Document.find(Object.assign({},df,{due_date:{$lt:today},status:{$nin:['Approved','Closed','Cancelled']}})).sort({days_open:-1});
      data.documents=await Document.find(df).sort({updated_at:-1});
      // Type × Discipline matrix for reports — respects active filter
      const tDA=await Document.aggregate([{$match:df},{$group:{_id:{type:'$type',discipline:'$discipline',status:'$status'},count:{$sum:1}}}]);
      tDA.forEach(r=>{const{type,discipline,status}=r._id;if(!type)return;if(!data.matrix[type])data.matrix[type]={};const disc=discipline||'Unspecified';if(!data.matrix[type][disc])data.matrix[type][disc]={};data.matrix[type][disc][status]=r.count;});
    }
